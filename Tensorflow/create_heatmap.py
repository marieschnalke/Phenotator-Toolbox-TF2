...
# Anpassung des Imports für Resampling
from PIL import Image
...
def create_heatmap_internal(input_images, background_image, output_folder, heatmap_width=100, max_val=None ,flower_list=None, min_score=0.5, overlay=True, output_image_width=1000, window=None,with_colorbar=True, create_tif=False):
    ...
    for i in progressbar.progressbar(range(len(input_images))):
        ...
        print(f"Predictions for {image_path}: {predictions}")  # Debug-Ausgabe
        ...
        for prediction in predictions:
            ...
            print(f"Processing prediction: {name}, score: {score}, center: ({center_x}, {center_y})")  # Debug-Ausgabe
            ...
            print(f"Heatmap coordinates: ({heatmap_x}, {heatmap_y})")  # Debug-Ausgabe
            ...
            if name == "lotus corniculatus":
                print(f"Adding 2.6 to heatmap at ({heatmap_x}, {heatmap_y})")  # Debug-Ausgabe
                heatmaps[name][heatmap_y][heatmap_x] += 2.6
                heatmaps["overall"][heatmap_y][heatmap_x] += 2.6
            else:
                print(f"Adding 1.0 to heatmap at ({heatmap_x}, {heatmap_y})")  # Debug-Ausgabe
    ...
    for heatmap_name in heatmaps:
        ...
        coverage_counter[coverage_counter == 0] = 1 # Verhindere Division durch 0
        heatmap = np.divide(heatmap, coverage_counter)
        heatmap = np.maximum(heatmap, 0)
        heatmap = np.clip(heatmap, 0, max_val)
        print(f"Heatmap '{heatmap_name}' Min: {np.min(heatmap)}, Max: {np.max(heatmap)}")
...
def save_heatmap_as_image(heatmap, output_path, background_image_path, background_image=None, output_image_width=1000, max_val=None, with_colorbar=True, create_tif=False):
    ...
    cmap = LinearSegmentedColormap.from_list("bla", float_color_ramp, N=256) # Mehr Farben für mehr Genauigkeit
    ...
    newIm = newIm.resize((int(output_image_width/width*height),output_image_width), Image.Resampling.LANCZOS)
    if background_image != None:
        newIm = newIm.resize(background_image.size, Image.Resampling.LANCZOS)
    ...
    if with_colorbar:
        ...
        imgplot.set_clim(0, max_val) # Setze die Farbskala so, dass sie nur positive Werte umfasst
