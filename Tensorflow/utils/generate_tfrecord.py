...
def class_text_to_weight(row_label, labels):
    ...
    total_flower_count = sum(labels.values())
...
def create_tf_example(group, path, labels):
    """
    Creates the tfrecord file. This function is largely adapted from the Tensorflow
    utilities.
    """
...
def make_tfrecords(input_csv, output_tf_record, images_folder, labels):
    ...
    Returns:
        None
    """
    writer = tf.io.TFRecordWriter(output_tf_record)
    ...
    writer.close()
    print(f'Successfully created the TFRecord file: {output_tf_record}')


if __name__ == '__main__':
    input_csv = 'path/to/your/input.csv'
    output_tf_record = 'path/to/your/output.tfrecord'
    ...
    make_tfrecords(input_csv, output_tf_record, images_folder, labels)
