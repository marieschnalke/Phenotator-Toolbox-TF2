...
@click.option('--tensorflow-tile-size', default=constants.tensorflow_tile_size, type=int)
def image_preprocessing(input_folder, test_split, validation_split, project_folder, tile_size, split_mode, min_instances, overlap, model_link, tensorflow_tile_size):
    """
    ...
    if check_inputs(folders=folders_to_check):
        ...
        image_preprocessing.convert_annotation_folders(input_folder, test_split, validation_split, project_folder, tile_size, split_mode, min_instances, overlap, model_link, tensorflow_tile_size)
...
@click.option('--create-tif', default=False, type=bool, help='If True, a geo tif will be created.', show_default=True)
def generate_heatmaps(predictions_folder, background_image, output_folder, heatmap_width, max_val, classname, min_score, overlay, output_image_width, generate_from_multiple, window, with_colorbar, create_tif):
    """
    ...
    if generate_from_multiple:
        if check_inputs(folders=[predictions_folder, output_folder], files=[background_image]):
            create_heatmap.create_heatmap_from_multiple(predictions_folder, background_image, output_folder, heatmap_width, max_val, classname, min_score, overlay, output_image_width, window, with_colorbar, create_tif)
    else:
        if check_inputs(folders=[predictions_folder, output_folder]):
            create_heatmap.create_heatmap(predictions_folder, output_folder, heatmap_width, max_val, classname, min_score, overlay, output_image_width, window, with_colorbar, create_tif)
...
@cli.command(short_help='Startet das Streamlit-Dashboard.')
@click.option('--dashboard-file', default='dashboard.py', help='Pfad zur Dashboard-Datei.')
def dashboard(dashboard_file):
    """
    Dieser Befehl startet das Streamlit-Dashboard.
    """
    # Starte Streamlit mit der angegebenen Dashboard-Datei
