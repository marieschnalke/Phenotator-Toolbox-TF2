...
This script carries out all kinds of evaluations on prediction results. 
Read the description of the main function evaluate(...) for more details.
"""
...
def evaluate(project_folder, input_folder, output_folder, iou_threshold=constants.iou_threshold, generate_visualizations=constants.generate_visualizations, should_print_confusion_matrix=constants.print_confusion_matrix, min_score=constants.min_confidence_score, visualize_info=constants.visualize_info):
    """
    Evaluates all predictions within the input_folder. The input folder should contain images alongside with prediction (imagename_prediction.json) and ground truth (imagename_ground_truth.json) files.
    Prints statistics such as precision, recall, mAP or f1 score for the overall prediction performance as well as for every class separately.
    
    Parameters:
        project_folder (str): path to the project folder
        input_folder (str): path of the input folder
        output_folder (str): path of the output folder. To this folder visualizations are printed.
        ...
        should_print_confusion_matrix (bool): If true, the confusion matrix is printed to the console in a format that can directly be imported into a latex table.
        min_score (float): The minimum score a prediction must have to be included in the evaluation
    ...
    object_detection_evaluator = object_detection_evaluation.ObjectDetectionEvaluator(
        labelmap, matching_iou_threshold=iou_threshold, evaluate_precision_recall=True, use_weighted_mean_ap=False)
    
    # COCO Evaluator hinzufügen
    coco_evaluator = coco_evaluation.CocoDetectionEvaluator(labelmap)
    ...
    for i in progressbar.progressbar(range(len(images))):
        ...
        gt_dict = {
            standard_fields.InputDataFields.groundtruth_boxes: bounding_boxes_gt,
            standard_fields.InputDataFields.groundtruth_classes: classes_gt,
            standard_fields.InputDataFields.groundtruth_difficult: np.full((len(bounding_boxes_gt)), False)
        }
        prediction_dict = {
            standard_fields.DetectionResultFields.detection_boxes: bounding_boxes_p,
            standard_fields.DetectionResultFields.detection_scores: scores_p,
            standard_fields.DetectionResultFields.detection_classes: classes_p
        }
        ...
        # COCO Evaluator ebenfalls aufrufen
        coco_evaluator.add_single_ground_truth_image_info(image_path, gt_dict)
        coco_evaluator.add_single_detected_image_info(image_path, prediction_dict)
    ...
    coco_evaluations = coco_evaluator.evaluate()

    flower_names.sort()
    stat_overall = {"tp": 0, "fp": 0, "fn": 0, "mAP": tensorflow_evaluations.get("Precision/mAP@" + str(iou_threshold) + "IOU", 0)}
    ...
    for flower_name in flower_names:
        ...
        # Korrigiere den Schlüssel, um das tatsächliche Schlüsselformat in tensorflow_evaluations zu verwenden
        key = "PerformanceByCategory/AP@" + str(iou_threshold) + "IOU/" + flower_name
        if key in tensorflow_evaluations:
            stat["mAP"] = tensorflow_evaluations[key]
        else:
            print(f"Warning: Key {key} not found in tensorflow_evaluations.")
            stat["mAP"] = 0  # oder handle es auf eine andere geeignete Weise

        print_stats(stat, flower_name, log_file)

    # Füge COCO-Evaluationsstatistiken hinzu
    coco_mAP = coco_evaluations.get('DetectionBoxes_Precision/mAP', 0)
    stat_overall['coco_mAP'] = coco_mAP
...
def filter_ground_truth(ground_truths, flower_names):
    """
    Helper function that filters all entries of the ground truth which names are not in the flower_names list.
...
def filter_predictions(predictions, min_score):
    """
    Helper function that removes all predictions from the predictions list that have a score of less than min_score
...
def get_flower_names_from_labelmap(labelmap_path):
    ...
    Returns:
        tuple: A tuple (flower_names, categories), where flower names is a list of strings containing all flower names and categories is a list of dicts in which each dict contains the id and the name of the flower
    """
    flower_names = []
    categories = []
...
def to_numpy_representations(annotations, categories):
    """
    Helper function that converts a list of annotations into a numpy representation that is required by tensorflow's evaluation classes
    
    Parameters:
        annotations (list): list of annotations
        categories (list): list of dicts in which each dict contains the id and the name of the flower
    
    Returns:
        tuple: numpy arrays of all bounding_boxes, all classes and all scores of the provided annotations
    """
